...
export default function VendorDetailPage() {
  ...
  // When the contact panel opens, pre-fill the email field with user's email
  ...
  const favMutation = useMutation({
    mutationFn: () => isFav ? api.delete('/favorites/' + id) : api.post('/favorites/' + id),
    onSuccess: () => { qc.invalidateQueries(['favorites']); toast.success(isFav ? 'Removed' : 'Saved to favourites'); }
  });

  const contactMutation = useMutation({
    mutationFn: () => {
      // Guarantee contactEmail is always a valid email before sending
      const emailToSend = contactForm.contactEmail.trim() || user?.email || '';
      if (!emailToSend) {
      ...
      return api.post('/contact', {
        ...
        ...(contactForm.eventId ? { eventId: contactForm.eventId } : {}),
      });
    },
    onSuccess: () => {
      toast.success('Inquiry sent!');
      setShowContact(false);
      setContactForm({ message: '', contactEmail: user?.email || '', eventId: '' });
    },
    onError: (e) => toast.error(e.message || e.response?.data?.message || 'Failed to send'),
  });
  ...
  return (
    <div className="max-w-5xl mx-auto px-4 sm:px-6 py-8">
      <Link to="/vendors" className="inline-flex items-center gap-2 text-sm text-gray-500 hover:text-gray-700 mb-6">
        ...
        <div className="lg:col-span-2">
          <div className="h-64 sm:h-80 rounded-2xl overflow-hidden bg-gray-100 mb-6">
            {vendor.imageUrl
              ...
              : <div className="w-full h-full bg-gradient-to-br from-brand-100 to-red-100 flex items-center justify-center text-6xl">🎪</div>
            }
          </div>
            ...
            <div className="flex gap-2 mt-2">
              <button
                ...
                className={`w-10 h-10 rounded-xl border flex items-center justify-center transition-colors
                  ${inCompare ? 'bg-gray-900 text-white border-gray-900' : 'border-gray-200 text-gray-600 hover:border-gray-400'}`}>
              ...
              {user && (
                <button
                  ...
                  className={`w-10 h-10 rounded-xl border flex items-center justify-center transition-colors
                    ${isFav ? 'bg-brand-500 text-white border-brand-500' : 'border-gray-200 text-gray-600 hover:border-gray-400'}`}>
        ...
        <div>
          <div className="card p-6 sticky top-24">
            <p className="text-2xl font-bold text-gray-900 mb-1">
              RM {Number(vendor.minPrice).toLocaleString()} – RM {Number(vendor.maxPrice).toLocaleString()}
            </p>
            ...
            {user ? (
              <>
                <button
                  ...
                  <Send size={16} />{showContact ? 'Cancel' : 'Send Inquiry'}
                </button>
                {showContact && (
                  <div className="mt-4 space-y-3">
                    ...
                    <div>
                      <label className="label text-xs">Link to Event (optional)</label>
                      <select
                        className="input text-sm"
                        value={contactForm.eventId}
                        onChange={(e) => setContactForm({ ...contactForm, eventId: e.target.value })}>
                    ...
                    <button
                      ...
                      disabled={contactMutation.isLoading || !contactForm.message.trim()}
                      className="btn-primary w-full text-sm">
                      {contactMutation.isLoading ? 'Sending...' : 'Send Inquiry'}
                    </button>
                  </div>
                )}
              </>
